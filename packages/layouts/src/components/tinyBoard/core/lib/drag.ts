...
import { BoardInstance, Layout, LayoutRect, Point } from '../types'
...
const getDragLimitHandler = <T>(
  ...
  const dashboardWidth = instance.getWidth()
  const { top, left, width } = boxRect
  const minX = -left
  const minY = -top
  const maxX = dashboardWidth - left - width
  const maxY = Infinity
...
/**
 * 仪表板内容块支持拖拽功能
 * @param config DragOptions
 */
export default function createDragProps<T>(config: DragOptions<T>) {
  const { instance, onDragStart = noop, onDrag = noop, onDragEnd = noop, noOverflow = false } = config
  return (item: Layout<T>) => ({
    ...
    onMouseDown(event: React.MouseEvent<HTMLElement, MouseEvent>): void {
      ...
      DragMove(null, {
        once: true,
        data: { id },
        onMove: ({ event, vector, data, initial }) => {
          if (initial) {
            itemRect = instance.activate()

            if (noOverflow) {
              dragLimit = getDragLimitHandler(itemRect, instance)
            } else {
