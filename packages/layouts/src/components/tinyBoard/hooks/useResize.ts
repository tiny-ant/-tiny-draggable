...
import { Direction, Layout, LayoutRect, InstanceRef, Point, SizeLimit, LayoutId } from '../types'

type ResizeOptions<T> = {
  /**
   * 当仪表板内部的某个块开始被resize时触发
   * @param event 鼠标事件对象(mousedown)
   * @param item 被resize的块的数据
...
export default function useResize<T>(config: ResizeOptions<T>) {
  const {
    onResizeStart = noop,
    onResize = noop,
    onResizeEnd = noop,
  ...
  return (instanceRef: InstanceRef<T>) => {
    ...
    // 一个闭包，每个Layout元素都有自己的rect计算数据，这样在同时拖动多个元素时才可以正常工作
    // TODO: 如果是多选resize如何支持？！！！
    instanceRef.current.getResizeProps = (item: Layout<T>) => {
      return {
        ...
        onMouseDown(event: React.MouseEvent<HTMLElement, MouseEvent>): void {
          ...
          if (onResizeStart(event.nativeEvent, item) === false) {
            return
          }
          ...
          // resizeSetter = instanceRef.current.getResizeSetter(direction)
          ...
          DragMove(null, {
            ...
            onMove: ({ event, initial, vector }) => {
              ...
              onResize(event, item, vector)
              resizeHandler(vector)
              instanceRef.current.reLayout(itemRect)
            },
            onEnd: ({ event, initial, vector }) => {
              onResizeEnd(event, item, vector)
              ...
              resizeHandler(vector)
              instanceRef.current.commitLayout(itemRect)
            },
          }).start({ x, y })
        },
      }
